...
const TeamTasks = () => {
  ...
  const fetchTeamTasks = async () => {
    try {
      ...
      const response = await api.get(`/projects/team/tasks?${params.toString()}`);
      setTasks(response.data.data.tasks);
      setTasksByAssignee(response.data.data.tasksByAssignee);
      setError(null);
    } catch (err) {
  ...
  if (loading) {
    return <LoadingSpinner text="Loading team tasks..." />;
  }
          ...
          <div className="col-12">
            <div className="d-flex justify-content-between align-items-center mb-4">
              <div>
                ...
                <p className="text-muted mb-0">
                  All tasks for your team ({tasks.length} total)
                </p>
              </div>
            ...
            {tasks.length === 0 ? (
              <div className="text-center py-5">
                ...
                <h4 className="text-muted">No Team Tasks Found</h4>
              ...
              <div className="row">
                {/* Tasks by Assignee View */}
                {Object.entries(tasksByAssignee).map(([assigneeId, assigneeData]) => (
                  <div key={assigneeId} className="col-12 mb-4">
                      ...
                      <div className="card-header bg-light">
                        <div className="d-flex justify-content-between align-items-center">
                          ...
                          <span className="badge bg-primary">
                            {assigneeData.tasks.length} tasks
                          </span>
                        </div>
                      </div>
                      <div className="card-body">
                        <div className="row">
                          {assigneeData.tasks.map((task) => (
                            <div key={task._id} className="col-lg-6 col-xl-4 mb-3">
                              <div className={`card h-100 ${isTaskOverdue(task.dueDate, task.status) ? 'border-danger' : ''}`}>
                                <div className="card-body">
                                  ...
                                  {task.dueDate && (
                                    <div className="mb-2">
                                      <small className={`${isTaskOverdue(task.dueDate, task.status) ? 'text-danger fw-bold' : 'text-muted'}`}>
                                        <i className={`fas fa-calendar ${isTaskOverdue(task.dueDate, task.status) ? 'text-danger' : ''} me-1`}></i>
