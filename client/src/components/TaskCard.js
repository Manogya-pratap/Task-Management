    ...
    switch (task.status) {
      case 'scheduled':
        return {
          ...
          text: task.scheduledDate ? formatDate(task.scheduledDate) : 'Scheduled',
          color: 'info'
        };
      case 'in_progress':
        return {
          icon: 'fas fa-play-circle',
          text: task.startDate ? `Started ${formatDate(task.startDate)}` : 'In Progress',
          color: 'warning'
        };
      case 'completed':
        return {
          icon: 'fas fa-check-circle',
          text: task.completedDate ? `Done ${formatDate(task.completedDate)}` : 'Completed',
          color: 'success'
        };
      default:
        return {
  ...
  return (
    ...
    >
      ...
      <div className="card-body p-3 position-relative">
        ...
        <div className="d-flex justify-content-between align-items-start mb-2">
          <div className="flex-grow-1">
            <h6 className="card-title mb-1 fw-bold task-title" style={{ fontSize: '0.95rem' }}>
              {task.title}
            </h6>
            {task.description && (
              <p className="card-text text-muted small mb-2 task-description" style={{ fontSize: '0.8rem' }}>
        ...
        <div className="task-metadata">
          ...
          {/* Assigned User */}
          {task.assignedTo && (
            <div className="d-flex align-items-center mb-2 user-info">
              <div className="user-avatar me-2">
                <i className="fas fa-user-circle text-primary" style={{ fontSize: '1.2rem' }}></i>
              </div>
          ...
          {/* Project Information */}
          {task.projectId && (
            <div className="d-flex align-items-center mb-2 project-info">
              <i className="fas fa-folder text-muted me-2" style={{ fontSize: '0.8rem' }}></i>
          ...
          {isOverdue() && (
            <div className="d-flex align-items-center overdue-warning">
              <i className="fas fa-exclamation-triangle text-danger me-2 pulse-animation" style={{ fontSize: '0.8rem' }}></i>
          ...
          {task.dueDate && !isOverdue() && task.status !== 'completed' && (
            <div className="d-flex align-items-center due-date-info">
              <i className="fas fa-clock text-muted me-2" style={{ fontSize: '0.8rem' }}></i>
              <small className="text-muted">
        ...
        {/* Task Footer */}
        <div className="d-flex justify-content-between align-items-center mt-3 pt-2 border-top task-footer">
          {/* Status Badge */}
          <StatusIndicator 
            status={task.status} 
            type="task" 
      ...
      <style jsx>{`
        ...
        .hover-overlay {
          background: linear-gradient(135deg, rgba(128, 0, 32, 0.8) 0%, rgba(160, 0, 42, 0.9) 100%);
          opacity: 0;
          transition: opacity 0.3s ease;
