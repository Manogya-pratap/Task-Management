...
export const StatusIndicator = ({
  ...
  const badgeSize =
    size === "lg" ? "fs-6 px-3 py-2" : size === "md" ? "px-2 py-1" : "";
    ...
    <div className={`position-relative d-inline-block ${className}`}>
      ...
      {showPercentage && (
        <div className="position-absolute top-50 start-50 translate-middle text-center">
          <div className="h4 mb-0" style={{ color }}>
            {percentage}%
          </div>
        ...
        <div className="d-flex justify-content-between align-items-center mt-2">
          <div className="d-flex gap-3 small">
            {segments.map((segment, index) => (
              <span key={index}>
                <i
                  className={`fas fa-square text-${segment.color || "primary"} me-1`}
                ></i>
      ...
      <div className="card-body text-center">
        ...
        {subtitle && (
          <small className={`text-${color}`}>
            {trend && (
              <i
                className={`fas ${trend.direction === "up" ? "fa-arrow-up" : "fa-arrow-down"} me-1`}
              ></i>
            )}
            {subtitle}
          </small>
...
export default statusIndicatorExports;
